...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q_data, k_data, v_data, attn_mask=None):
        ...
        N, S, E = q_data.shape
        N, T, E = v_data.shape
        query, key, value = self.Wq(q_data), self.Wk(k_data), self.Wv(v_data)
        query = query.reshape(N, S, self.n_head, self.head_dim).transpose(1, 2) # (N, H, S, E/H)
        key = key.reshape(N, T, self.n_head, self.head_dim).transpose(1, 2) # (N, H, T, E/H)
        value = value.reshape(N, T, self.n_head, self.head_dim).transpose(1, 2) # (N, H, T, E/H)
...
def multi_head_attention(query, key, value, attn_mask=None, dropout=0.1):
    ...
    attn_score = torch.matmul(query, key.transpose(2,3)) / np.sqrt(head_dim) # (N, H, S, T)
    if attn_mask is not None:
          attn_mask = attn_mask.unsqueeze(0).unsqueeze(0).repeat(N, n_head, 1, 1) # (N, H, S, T)
          attn_score = attn_score.masked_fill(attn_mask == 0, float('-inf'))
    

    attn_score = torch.softmax(attn_score, dim=-1)
    attn_drop = nn.Dropout(dropout)
    attn_score = attn_drop(attn_score)

    output = torch.matmul(attn_score, value).transpose(1, 2).reshape((N, S, E)) # (N, H, T, E/H)
