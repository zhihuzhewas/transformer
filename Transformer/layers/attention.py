...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Implement multi-headed attention using the equations given in             #
        # homework document.                                                       #
        # A few hints:                                                             #
        ...
        query = self.query(query).view(N, -1, self.n_head, self.head_dim).transpose(1, 2)
        # (N, T, E) => (N, T, H, E/H) => (N, H, T, E/H)
        key = self.key(key).view(N, -1, self.n_head, self.head_dim).transpose(1, 2)
        value = self.value(value).view(N, -1, self.n_head, self.head_dim).transpose(1, 2)

        # (N, H, S, T)
        attention = query @ key.transpose(-1, -2) / np.sqrt(self.head_dim)
        if attn_mask is not None:
            # (S, S) => (N, H, S, S)
            attn_mask = attn_mask.eq(0).unsqueeze(0).unsqueeze(1).repeat(N, self.n_head, 1, 1)
            attention.masked_fill_(attn_mask, float('-inf'))
